...
def _process_molecule(record_and_molecule) -> oechem.OEMol:
    """Convert a QC record and associated molecule to a result-tagged OEMol.

    The resulting OE molecule is tagged with the associated SMILES,
    final energy and record id."""
    ...
    final_energy = (
        record.get_final_energy() * qcelemental.constants.hartree2kcalmol
    )
...
def main(data_set, output):
    ...
    * "OpenFF Industry Benchmark Season 1 v1.0"
    ...
    The processed molecules tagged with information from the QC record,
    including the CMILES and QC energy, will be stored in a new
    `01-processed-qm.sdf` file and additionally information about the
    included records will be stored in `01-processed-qm.json`.
    """
    ...
    output_steam = oechem.oemolostream(f"{output}.sdf")

    final_record_ids = set()

    for i, oe_molecule in enumerate(processed_oe_molecules):
        final_record_ids.add(
            oechem.OEGetSDData(oe_molecule, "Record QCArchive")
        )
